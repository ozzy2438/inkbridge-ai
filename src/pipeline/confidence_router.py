...
class ConfidenceRouter:
    ...
    def _calculate_complexity(self, layout: PageLayout, quality: QualityReport) -> float:
        ...
        region_types = {line.region_type for line in layout.lines}
        score += (len(region_types) - 1) * 1.0

        # Crossed-out text
        crossed_out_count = sum(1 for line in layout.lines if line.is_crossed_out)
        score += crossed_out_count * 0.5

        # Margin insertions
        insertions = sum(
            1 for line in layout.lines if line.region_type == RegionType.MARGIN_INSERTION
        )
