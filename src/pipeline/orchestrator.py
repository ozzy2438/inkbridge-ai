...
class PipelineOrchestrator:
    ...
    async def process_single_page(
        ...
        routing = self.confidence_router.route(quality_report, layout, force_vlm=force_vlm)
        ...
        full_text = "\n".join(
            b["text"]
            for b in sorted(blocks, key=lambda x: x.get("reading_order", 0))
    ...
    async def _run_trocr(self, image_cv: np.ndarray, layout) -> tuple[list, list]:
        ...
        for line in layout.lines:
            ...
            cropped = image_cv[line.bbox.y1 : line.bbox.y2, line.bbox.x1 : line.bbox.x2]
