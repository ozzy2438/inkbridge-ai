...
class LayoutAnalyzer:
    ...
    def analyze(self, image: np.ndarray, page_id: str = "page_0") -> PageLayout:
        ...
        layout = PageLayout(
            ...
            height=h,
            lines=lines,
            reading_order=[line.reading_order for line in lines],
            metadata={
                "total_lines": len(lines),
                "handwriting_lines": sum(
                    1 for line in lines if line.region_type == RegionType.STUDENT_HANDWRITING
                ),
                "crossed_out_lines": sum(1 for line in lines if line.is_crossed_out),
            },
        )
    ...
    def _detect_crossouts(self, gray: np.ndarray, lines: list[TextLine]) -> list[TextLine]:
        ...
        for line in lines:
            ...
            if h_lines is not None:
                ...
                for hl in h_lines:
                    coordinates = np.asarray(hl).reshape(-1)
    ...
    def _establish_reading_order(self, lines: list[TextLine]) -> list[TextLine]:
        ...
        sorted_lines = sorted(lines, key=lambda line: (line.bbox.y1, line.bbox.x1))
